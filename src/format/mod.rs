...
pub fn input_with_start_offset<P: AsRef<Path>>(
    path: &P,
    offset: i64,
    format: CString,
) -> Result<context::Input, Error> {
    unsafe {
        ...
        println!("file extension: {:?}", format);
        let path = from_path(path.clone());

        // (*ps).interrupt_callback = interrupt::new(Box::new(closure)).interrupt;
        (*ps).skip_initial_bytes = offset;
        (*ps).iformat = av_find_input_format(format.as_ptr());
